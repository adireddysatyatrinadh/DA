...
from File2File import File2File
    ...
    def Execute(self,p_job_worker_cd,p_job_cd,p_job_step_cd,p_job_grp_cd):
        try:
            ...
            if (lv_job_program_action == "SYNC_TAB2TAB"):
                ...
                self.log.Write("BEGIN: PLSQL.ExecuteProcedure CALL")

                obj = PLSQL(self.log)
                jobStepStatus, errorCd, errorMsg = obj.ExecuteProcedure(lrowjs, self.wdb, self.p)
                self.log.Write("OUTPUT:")
                self.log.Write(jobStepStatus, errorCd, errorMsg)

                self.log.Write("END: PLSQL.ExecuteProcedure CALL")
            ...
            elif (lv_job_program_action == "EXEC_PLPGSQL"):

                self.log.Write("BEGIN: PLpgSQL.ExecuteFunction CALL")

                obj = PLpgSQL(self.log)
                jobStepStatus, errorCd, errorMsg = obj.ExecuteFunction(lrowjs, self.wdb, self.p)
                self.log.Write("OUTPUT:")
                self.log.Write(jobStepStatus, errorCd, errorMsg)

                self.log.Write("END: PLpgSQL.ExecuteFunction CALL")
            elif (lv_job_program_action == "COPY_FILE2FILE"):

                self.log.Write("BEGIN: File2File CALL")

                obj = File2File(self.log)
                jobStepStatus, errorCd, errorMsg = obj.CopyFile(lrowjs, self.wdb, self.p)
                self.log.Write("OUTPUT:")
                self.log.Write(jobStepStatus, errorCd, errorMsg)

                self.log.Write("END: File2File CALL")
